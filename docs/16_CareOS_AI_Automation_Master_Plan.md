...
Status: Partially ratified — provider ratified as D-013 (2026-08-01); §2.8 staff-lifecycle rows ratified as D-021 (2026-08-04); §2.9 verified-visit rows ratified with the D-022…D-030 set (2026-08-09), D-028 governing the T2 row; remaining §7 proposals open
Owner: Head of AI Product
Date: 2026-08-01 (amended 2026-08-04: §2.8 added per D-021; amended 2026-08-10: §2.9 added and built, per docs/17 §10–§11)
Cross-references: docs/00 (decision log), docs/07 (schema), docs/08 (API), docs/09 (security/vendor register), docs/10 (frontend), docs/11 (AI implementation spec), docs/15 (build plan), **docs/17 (verified visit & workforce intelligence)**, `supabase/migrations/0014_ai_plane.sql`, `supabase/migrations/0051_workforce_analytics.sql`, `supabase/migrations/0052_visit_ai_capabilities.sql`, `supabase/seeds/zz_ai.sql`, `apps/web/src/lib/ai/client.ts`, `apps/web/src/lib/ai/visit-intelligence.ts`
...
The plan below maps 45 workflows, assigns each a tier and an OpenAI primitive, sequences them into six waves, and shows the unit economics close: full deployment for a 100-staff, 300-client agency costs roughly $130–300/month in model spend against an estimated 240–290 staff-hours/week returned.
...
| X4 | NL analytics over the tenant ("clients with 2+ missed visits this month") | Owner/coordinator exports nothing — the query is simply not askable today | Auto (T1), read-only governed query templates; grammar-constrained tool args. **Doc 17 §11 extends this row** (its "`ops.nl_query` (extend X4)") with the verified-visit views — as a **new prompt version on the existing `analytics.query` capability**, not a second registry row: a duplicate would fork a live capability's kill switch and budget and leave two rows to disable in an incident. Not yet authored | Strict tools + Lark/regex grammars (never free SQL) | visit, schedule_exception, client, census aggregates, *(planned: `verified_visit`, `visit_exception_state`)* | 4 | L |
| X5 | **Daily Huddle Brief** (per-role morning operations brief: overnight call-outs, at-risk clients, coverage gaps, overdue obligations, expiring credentials, yesterday's exceptions, unsigned queue) | The coordinator morning scramble — "several hours each day" industry-wide (source: rosemarksystem.com); owner reads five panels and chases by phone; no formal huddle exists in home care | Auto (T0 generate + T1 notify); every item deep-links; all facts from deterministic queries, LLM narrates and ranks only. **Doc 17 §11 extends this row** (its "`ops.daily_brief` (extend X5)") with the verified-visit counters and the §10 feature set — as a **new prompt version on the existing `huddle.brief` capability**, not a second registry row (see X4). Not yet authored | Batch API nightly (50% off) on gpt-5.6-terra; per-role render | schedule_exception, visit_event, cadence_obligation_status, credential_expiry, signature gaps, attention-panel queries, *(planned: `app.workforce_features`)* | 20 | M |
...
| # | Workflow | Manual cost today | AI treatment | OpenAI primitive | Data it feeds on | Impact (hrs/wk) | Size |
|---|---|---|---|---|---|---|---|
| B1 | EVV exception pre-adjudication (clock pair vs schedule vs auth window reconciliation, drafted fix narratives) | EVV↔billing↔auth misalignment is the biggest claims-error source (source: vertexsystems.com); 11.8% first-pass denial rate industry-wide, $25–181 rework per denial (source: Experian State of Claims 2025; aptarro.com) | Det rules classify every exception; Draft+approve (T2) for the documented correction narrative (manual corrections require reasons); billing export stays gated to EVV-verified visits — deterministic, per FR-AI-070. **The deterministic half is now built** — the five `app.detect_*` rules and `app.sweep_visit_exceptions` (migration 0047) classify every exception, and V1 narrates the queue; what remains here is the drafted *correction narrative* | luna for narratives only | `visit_event`, `visit_exception`, visit windows, `capture_source='manual'` anomalies | 6 | M |
...
### 2.9 Verified visit & workforce intelligence (V)

Ratified with the D-022…D-030 set (2026-08-09) and **built**: the deterministic half is migrations 0043–0052, the four capabilities below are registered by 0052 with a pinned model, a monthly cap, a kill switch and a versioned prompt, and the narration lives in `apps/web/src/lib/ai/visit-intelligence.ts` on the same pattern as `huddle.ts`. Docs/17 §10–§11 is the pinned contract. Note that §11's other two rows are **not** new capabilities and were deliberately not registered: they are this document's X5 and X4 (`huddle.brief`, `analytics.query`), and extending a live capability means a new prompt version, never a second row under an alias.

These four rows are the purest expression of §1's second claim. The verified visit produces an unusual amount of *already-true* structure — who was scheduled, who arrived, where, when, how long, and what the rules engine found wrong — and **all of it is computed in SQL or PostGIS before any model is consulted**. Distance, geofence membership, minutes worked, overlap, impossible travel, lateness under policy, EVV element completeness, approved hours and the trust score are facts. Queue order is arithmetic too: the exception inbox ranks by severity × recency × payroll impact × client risk in pure TypeScript, and no model may reorder it. What the model does here is narrate and rank prose — nothing else — which is why three of the four sit at T1 with no disposer: there is no act to dispose of.

| # | Workflow | Manual cost today | AI treatment | OpenAI primitive | Data it feeds on | Impact (hrs/wk) | Size |
|---|---|---|---|---|---|---|---|
| V1 | **Visit exception triage** (`visit.exception_triage`) — narrate the ranked exception queue: what the engine found, what it blocks, the next step in the app | Coordinator reads an undifferentiated list of clock anomalies and works out for each one what it means and whether it stops a paycheque | Det+narrate (**T1**, no disposer — the ranking is deterministic, the model writes only the *why*) | luna + structured outputs | `visit_exception_state` + `verified_visit`, read under the caller's RLS — enums, counts, ages and per-request opaque handles only. (Doc 17 §11 also permits the §10 feature set here; the built capability does not read it, and narrower is the right side to err on) | 4 | shipped |
| V2 | **Weekly workforce report** (`workforce.weekly_report`) — the operations narrative: adherence, lateness, exception and overtime patterns across the workforce | No forward or trend view exists; patterns are noticed anecdotally or not at all | Det+narrate (**T1**) — patterns only; characterising an individual is V4, a different capability with a different tier | terra + structured outputs | `app.workforce_features` **and nothing else** | 2 | shipped |
...
- **No coordinates, ever, and no raw accuracy radius** (D-030). Location is PHI about the client and workforce PII about the caregiver simultaneously; a per-person analysis fed raw positions would be surveillance with a summarisation step. 0051's pgTAP asserts neither leaves the function or its supporting views.
- **No free text.** No notes, no reasons, no client details, no clinical content. Everything the model sees is a count, a rate, a minute total or an enum — which is what makes the "never invent a number" guardrail checkable: a sentence carrying a numeric token that is not in the facts is dropped before anyone reads it.
- **It refuses at the perimeter, not in the prompt.** The function requires AAL2 and `workforce.read`, is pinned to the caller's tenant, rejects a foreign caregiver and an incoherent window, and its supporting views are `security_invoker` so rows compose through the caller's own RLS (invariant 9).

Bypassing it — reading `visit_event`, joining names, passing a note into a prompt "just for context" — is not a shortcut but a different capability with different obligations, and the registry, the minimizer and 0051's assertions are all arranged so that doing it accidentally is hard.

**Why V4 is T2 and stays there.** It characterises an individual employee, which is exactly the artifact that erodes invariant 8 and D-021's "the system never proposes termination" by accident. So: a licensed human disposer is **required in the database** — 0052's pgTAP proves the registry refuses to register the key without one, on INSERT and on any later UPDATE — the capability writes no employment record (the only row its path can create is the `ai_interaction` ledger entry), it runs only when a person clicks for it and never on a schedule or on load, its prompt is instructed to say *"the sample is too small to characterise a person"* and stop when the window is thin, and a second guardrail discards the entire draft if any sentence reads as proposing discipline, separation, probation or a character judgement. The draft is never shown to the person it describes, and the capability row ships `enabled = false`.

**Estimated total: ~244–294 staff-hours/week returned** for a 100-staff agency at full deployment (~6–7% of total paid hours), dominated by voice-to-note. Roughly 60% of mapped items are drafting/chasing workflows where the data already sits in-system and the human glue is a phone/text loop. For the §2.9 rows in particular the honest attribution is that the hours are returned by the **deterministic engines** — the detectors, the ranking, the approved-hours ledger — and the narration makes them legible; strip the model out and the figures still render (§6, "blast-radius controls").
...
**The §2.9 rows shipped ahead of this sequence, and the ordering logic is why that was safe.** They arrived with the verified-visit layer (docs/17) rather than in a wave slot, but they take no outbound action, execute nothing, and write no domain row: V1–V3 are T1 read-only narration over deterministic output, and V4 — the only capability that characterises a person — ships with its registry row **disabled**, requires a human disposer enforced by a database constraint, and produces a draft that lives in one browser until a human does something with it. Nothing in §2.9 needed the Approvals Inbox because nothing in §2.9 proposes an act to approve. The Wave-0 floor they do depend on — registry-versioned prompts, per-capability kill switches, budget caps, the minimizer, the single `client.ts` chokepoint — was already in place, which is the whole argument for building the floor first.
...
| Visit/workforce narration (§2.9 V1–V4: exception triage on queue open, weekly report, payroll-readiness on close, occasional profile draft) | ~250 calls | luna (V1) + terra (V2–V4), sync | 3K in / 0.7K out | $4 |
...
**Tier enforcement.** T2/T3 human-disposer requirements are enforced in Postgres, not in application politeness — there is no code path that executes a gated capability without a disposition row, and the red-team CI suite (Wave 4) attacks exactly that. Anti-capabilities stay anti: background-check adjudication is always human (FR-AI-052); billing export is deterministically gated to EVV-verified visits (FR-AI-070); geofence exceptions record and flag but never block care (FR-AI-032) — the built clock RPC offers a reason and carries on rather than refusing the visit, and self-approval of hours is refused by the RPC *and* by a CHECK. **Characterising an employee is the newest member of that list:** `visit.operational_profile` (§2.9 V4) cannot be registered without a human disposer, cannot write an employment record, and has a guardrail that discards any sentence proposing discipline or separation. AlayaFlow's auto-resolution of verification failures is the cautionary tale, not the benchmark.
...
**PHI containment.** Allowlist minimizer + redaction manifest + planted-PHI canaries precede any PHI-bearing capability; IDs travel and content is refetched under RLS; retrieval executes as the requester; no PHI in Batch until the BAA/Safety-Retention lane is live; training uploads are synthetic-or-de-identified only, forever (§3.3, §3.5). **Location is a closed case (D-030):** a caregiver's coordinates at a visit reveal the client's home address, so they are PHI about the client and PII about the caregiver at once — they never enter a prompt, a payload, a notification or telemetry, workforce analysis reads only `app.workforce_features` (§2.9), and the field UI is handed a distance *bucket* rather than a number. The database-layer canaries for this are built and asserted (docs/12 §6); the egress-layer canaries are not yet, and that gap is named rather than assumed closed.
...
Status of this list: item 1 is **ratified** (D-013); item 3 is **resolved** (D-025, 2026-08-09) — though not as proposed, see below; the §2.8 staff-lifecycle rows this doc did not originally map were proposed and ratified separately as **D-021** (2026-08-04), alongside **D-020** (machine AAL2 — the agent-identity basis the H rows and every agent-run capability in §2 execute under); the §2.9 verified-visit rows were ratified with the **D-022…D-030** set (2026-08-09). Items 2, 4 and 5 remain open proposals.
...
3. ~~**D-0XX Client geocoding.**~~ **Resolved as D-025 (2026-08-09), and not the way this item proposed.** Stored geocodes landed, but *no geocoding vendor was added*: address normalisation is a pure `immutable` SQL function (`app.normalize_address`) and a coordinate counts as verified only with a named human's attestation (`service_location_version.verified_by` + `verified_at`, CHECK-enforced). The vendor seam exists and is disabled, because client addresses are PHI and Google Maps Platform — the API this item implied — is not a BAA-eligible service. Registering a BAA-eligible provider later is a `geo_source='provider'` row and an adapter, with no schema change and no rework of B1/G2.
